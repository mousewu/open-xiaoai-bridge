---
name: audio-player
description: Search and play audio from a local music library on the XiaoAI speaker via the OpenXiaoAI HTTP API, with playlist queue support and optional YouTube audio fallback. Also drives a persistent "English listening plan" (萌鸡小队+小猪佩奇 交替循环 20 遍，断点续播). Use when the user wants to play music, podcasts, or kids' audio content on the speaker. Triggers on queries like "放歌", "放一首", "播放音乐", "点歌", "听歌", "播放播客", "放本地音乐", "播放专辑", "整季播放", "下一首", "停止播放", "继续英语听力", "开始英语听力", "英语听力计划", "放萌鸡和佩奇", "英语听力放到哪了", "audio-player".
---

# Audio Player

通过小爱音箱检索并播放本地音频库（默认 /Volumes/music）中的内容，支持模糊/拼音检索、目录整体连播（歌单队列）、跳曲/停止控制。本地库没有的内容可从 YouTube 下载音轨兜底。

## 前置配置

```bash
OPENXIAOAI_BASE_URL="http://192.168.x.x:9092"   # OpenXiaoAI 服务地址（默认 http://127.0.0.1:9092）
MUSIC_LIBRARY_DIR="/Volumes/music"               # 音频库根目录（默认 /Volumes/music）
AUDIO_PLAYER_CACHE="/Volumes/music/YouTube"      # YouTube 下载目录（默认在音频库内，下载后可直接检索点播）
```
...
# 下载文件存到 /Volumes/music/YouTube/"标题 [ID].mp3"，之后 search_local.py 可直接按标题搜到，
# 同一视频重复请求会复用已下载文件
```

## 英语听力循环计划（续播，语音触发）

用户给孩子定了一个长期计划：把 **萌鸡小队英文版第一季 + 小猪佩奇英语第一季**（各 52 集）
交替合成的列表 `/Volumes/music/英语听力计划_萌鸡+佩奇交替/` **循环播放 20 遍**。
进度持久化在 `~/.audio-player/progress.json`，被打断后能从**当前这一集**续上、跨时段累计。
另有 launchd 定时任务在工作日固定时段自动开播/收尾。用户可能想用语音临时开始/停止/查进度。

**听到下列意图时，直接运行对应命令，不要走 search_local：**

| 用户说（含义） | 运行 |
|---|---|
| "继续英语听力" / "开始英语听力(计划)" / "放萌鸡和佩奇" / "接着放英语听力" | `~/.audio-player/plan.sh start` |
| "停(一下)英语听力" / "暂停英语听力" / "英语听力先停" | `~/.audio-player/plan.sh stop` |
| "英语听力放到哪了" / "听力进度" / "还剩几遍" | `~/.audio-player/plan.sh status`（读 JSON 后用 xiaoai-tts 口播，如"第 3 遍，第 42 集，已完成 15%"） |

- `plan.sh start` 会**从上次断点续播**（不是重头），跑满 20 遍自动停；进度与定时任务共用同一计数器，
  所以语音临时加的这一场也**计入**总进度、完全连续。
- `plan.sh` 是自洽的（内部用绝对路径调用本仓库的 `play.py --resume`，并设好 PATH/ffprobe/服务地址），
  无需 cd、无需额外环境。
- **边界**：只有明确的"英语听力/听力计划/萌鸡+佩奇"这类计划意图才走 plan.sh。
  若用户只说"放萌鸡小队"或"放小猪佩奇"（单部片子），按普通流程用 `search_local.py` 找对应目录、
  `play.py` 播放该目录即可，**不要**触发循环计划。
- **语音确认要在启动前**：`plan.sh start` 内部调 play.py 会结束语音会话（见下方⚠️），
  想给"好的，继续英语听力"这类回应，先用 xiaoai-tts `--blocking` 播完再运行 start。
  停止/查进度不启动播放，可正常口播。
